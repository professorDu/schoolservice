    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">宿舍管理员修改</h4>
            </div>
            <div class="modal-body">
                <form class="form-horizontal">
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="dormName" class="form-control"
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="dormUsername" class="form-control"
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="dormPassword" class="form-control"
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <label class="radio-inline"> <input type="radio"
                                                                name="dormGender" value="M"
                                                                checked="checked"> 男
                            </label> <label class="radio-inline"> <input type="radio"
                                                                         name="dormGender" value="F">
                            女
                        </label>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="dormTel" class="form-control"
                                   id="dormTel_Update_input" placeholder="18896741234"> <span
                                class="help-block"></span>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-2 control-label">管理楼栋</label>
                        <div class="col-sm-4">
                            <!-- 部门提交部门id即可 -->
                            <select class="form-control" name="apartPid"
                                    id="apart_Update_select">
                            </select>
                        </div>
                    </div>
                </form>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">宿舍管理员添加</h4>
            </div>
            <div class="modal-body">
                <form class="form-horizontal">
                    <div class="form-group">
...
<script type="text/javascript">
    ...
    function build_managerinfo_table(result) {
        ...
        $.each(managerinfo, function (index, item) {
            ...
            // var dormIdTd = $("<td></td>").append(item.dormId);
            var dormIdTd = $("<td name=''></td>").append(index + 1);
            dormIdTd.attr("name", item.dormId);
            var dormNameTd = $("<td></td>").append(item.dormName);
            var dormUsernameTd = $("<td></td>").append(item.dormUsername);
            var dormGenderTd = $("<td></td>").append(item.dormGender == 'M' ? "男" : "女");
            var dormTelTd = $("<td></td>").append(item.dormTel);
            ...
            var delBtn = $("<button></button>").addClass("btn btn-danger btn-sm delete_btnmana")
                .append($("<span></span>").addClass("glyphicon glyphicon-trash")).append("删除");
            //为删除按钮添加一个自定义的属性来表示当前删除的员工id
            delBtn.attr("del-id", item.dormId);
    ...
    $("#emp_add_modal_btn").click(function () {
        ...
        //发送ajax请求，查出楼栋信息，显示在下拉列表中
        getApart("#empAddModal select");
        //弹出模态框
        $("#empAddModal").modal({
            backdrop: "static"
        });
    ...
    //单个删除
    $(document).on("click", ".delete_btnmana", function () {
        //判断计时器是否处于关闭状态
        if (time == 0) {
            time = 1; //设定间隔时间（秒）
            //启动计时器，倒计时time秒后自动关闭计时器。
            ...
            if (confirm("确认删除【" + empName + "】吗？")) {
                //确认，发送ajax请求删除即可
                $.ajax({
                    url: "${APP_PATH}/dormmanager/" + empId,
                    type: "DELETE",
                    async: false,
                    cache: false,
                    dataType: "text",
    ...
    $("#emp_delete_all_btn").click(function () {
        ...
        $.each($(".check_item:checked"), function () {
            ...
            //组装员工id字符串
            del_idstr += $(this).parents("tr").find("td:eq(1)").attr("name") + "-";
        });
        //去除empNames多余的,
        ...
        if (confirm("确认删除【" + empNames + "】吗？")) {
            //发送ajax请求删除
            $.ajax({
                url: "${APP_PATH}/dormmanager/" + del_idstr,
                type: "DELETE",
                success: function (result) {
                    alert(result.msg);
                    //回到当前页面
